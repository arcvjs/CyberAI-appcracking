...
1. Click **Launch game** while the tool says **Protected**. Vapor opens; click Play to show the expired trial. If the account was previously activated, use `App/Presenter/DemoControl.exe expired` before presenting.
2. Close Vapor and Afterlight. Click **Apply replacement**. The tool backs up and verifies the original SDK, then replaces only `App/Vaporworks.dll`.
...
The account license is not altered by Apply or Restore. The expired-trial state remains in the service. Backups are stored in `App/.sdk-backup/`. The tool rejects unknown game/SDK builds and damaged backups. Close the game and launcher before changing files.
